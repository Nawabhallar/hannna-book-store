...
function UserMenu() {
  // call useAuth defensively — if no Provider is present, useAuth() may return undefined
  const auth = useAuth();
  const currentUser = auth?.currentUser ?? null;
  const fallback = "/fav-icon.png";
  // If admin token is present, show admin as a hardcoded identity
  const isAdmin = !!localStorage.getItem("token");
  const name = isAdmin
    ? "admin"
    : currentUser?.displayName || currentUser?.email || "User";
  const role = isAdmin ? "Administrator" : currentUser ? "User" : "";
  const photo = isAdmin ? fallback : currentUser?.photoURL || fallback;
